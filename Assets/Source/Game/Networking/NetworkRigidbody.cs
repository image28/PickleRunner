		...
		if (m_BufferedState[0].timestamp > interpolationTime)
		{
			...
			for (int i = 0; i < m_TimestampCount; i++)
			{
				if (m_BufferedState[i].timestamp <= interpolationTime || i == m_TimestampCount - 1)
				{
					...
					transform.localPosition = Vector3.Lerp (transform.localPosition,Vector3.Lerp (lhs.pos, rhs.pos, t),interpolationConstant);
					transform.localRotation = Quaternion.Slerp (transform.localRotation, Quaternion.Slerp(lhs.rot, rhs.rot, t), interpolationConstant);
					GetComponent<Rigidbody>().velocity = Vector3.Lerp (GetComponent<Rigidbody>().velocity, Vector3.Lerp (lhs.velocity, rhs.velocity, t), interpolationConstant);
					GetComponent<Rigidbody>().angularVelocity = Vector3.Lerp (GetComponent<Rigidbody>().angularVelocity, Vector3.Lerp (lhs.angularVelocity, rhs.angularVelocity, t), interpolationConstant);
					return;
				}
			}
